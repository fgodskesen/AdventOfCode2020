...
function Get-VisibleNeighbours {
    ...
    param (
        ...
        # Left
        for ($col = 0; $col -lt $ColNum; $col++) {
            if ($Seating[$RowNum][$col] -eq [SeatState]::Occupied) {
                $hits++
            }
        }

        # Right
        for ($col = $ColNum + 1; $col -gt $ColNum -and $col -lt $Seating[0].Count; $col++) {
            if ($Seating[$RowNum][$col] -eq [SeatState]::Occupied) {
                $hits++
            }
        }
        ...
        # Up
        for ($row = 0; $row -lt $RowNum; $row++) {
            if ($Seating[$row][$ColNum] -eq [SeatState]::Occupied) {
                $hits++
            }
        }

        # Down
        for ($row = $RowNum + 1; $row -gt $RowNum -and $row -lt $Seating.Count; $row++) {
            if ($Seating[$row][$ColNum] -eq [SeatState]::Occupied) {
                $hits++
            }
        }
